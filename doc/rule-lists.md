...
The rules are stored in `backend/src/diversifix_server/data/suggestions_editable.csv`. Unfortunately, the CSV file cannot be opened or imported correctly into Microsoft Excel, and it also exports into a not-so-clean format, so we advise to edit the file in a simple text editor or a CSV editor.
...
The rule lists will automatically be processed, so the rules can be used faster when the app is running. The processing happens in the `backend/src/diversifix_server/prepare_list.py` module, and new/changed/deleted rules are automatically processed when the server (re)starts. The results are stored in `backend/src/diversifix_server/data/suggestions_processed.csv`. That file should not be edited manually.
